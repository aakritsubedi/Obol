...
import { formatCurrency, formatTokens, type MoneyDisplay, moneyDisplay, numberValue } from "../lib/format";
...
function tooltipFor(
  ...
  return [
    ...
    `${formatCurrency(day.cost, locale, display)} total cost`,
  ];
}
...
export function buildContributionCalendar(
  ...
  now: Date,
  locale?: string,
  display: MoneyDisplay = moneyDisplay(),
  ...
  for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
    ...
    days.push({
      ...
      tooltip: tooltipFor(base, dataStart, locale, display),
    });
  }
