...
export default function Ticker({ summary }: Props) {
  ...
  return (
    <aside
      className="fixed bottom-4 left-1/2 z-30 flex min-h-11 w-[calc(100%-36px)] max-w-[780px] -translate-x-1/2 items-center gap-3.5 overflow-hidden rounded-full bg-ink px-3.5 py-2 text-surface shadow-[0_12px_30px_rgba(0,0,0,.18)]"
      aria-label="Today's provider spend"
    >
      <span className="flex shrink-0 border-r border-surface/20 pr-3.5 text-[10px] font-semibold uppercase text-surface/70">
        Today
      </span>
      <div className="flex min-w-0 items-center gap-[18px] overflow-x-auto">
        {providers.map((provider) => {
          ...
          const active = provider.totalCost > 0;
          return (
            <span
