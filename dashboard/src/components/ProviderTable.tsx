          ...
          {providers.length === 1 ? (
            <div className="flex items-center gap-2 text-xs text-muted">
              ...
              {providers[0].totalCost === 0 && (
                <span className="rounded-full bg-wash px-2 py-0.5 text-[10px] font-semibold text-subtle">Free</span>
              )}
              <span>· {formatTokens(providers[0].totalTokens)} tokens · {formatCurrency(providers[0].totalCost)}</span>
            </div>
            ...
            return (
              <div className="grid grid-cols-[30px_minmax(0,1fr)_auto] items-center gap-[11px]" key={provider.agent}>
                ...
                <div className="min-w-0">
                  <div className="flex items-center justify-between gap-3.5 text-xs">
                    <span className="flex items-center gap-2">
                      ...
                      {free && <span className="rounded-full bg-wash px-2 py-0.5 text-[10px] font-semibold text-subtle">Free</span>}
                    </span>
                    <span className="text-[11px] tabular-nums text-muted">{free ? "$0.00" : `${share.toFixed(0)}%`}</span>
                  </div>
                  <div className="my-2 h-1 overflow-hidden rounded-full bg-track">
                    <span
                      className="block h-full rounded-full"
                      style={{ width: `${Math.min(100, Math.max(0, barShare))}%`, backgroundColor: free ? "var(--color-subtle)" : color }}
                    />
                  </div>
