...
final class CurrencyController: ObservableObject {
    ...
    private let onCurrencySelected: (String, Double?) -> Void

    init(
        client: CurrencyClient = CurrencyClient(),
        defaults: CurrencyDefaults = CurrencyDefaults(),
        onCurrencySelected: @escaping (String, Double?) -> Void = { _, _ in }
    ) {
        self.client = client
    ...
    func adopt(code: String, rate sharedRate: Double? = nil) {
        ...
        if sharedRate == nil, option.code != CurrencyOption.usd.code, let cachedRate = rate?.rate {
            // The daemon's code wins on adoption. Once it has confirmed the
            // same code, fill in an older daemon's missing rate from the local
            // cache so the dashboard can use the exact native conversion too.
            onCurrencySelected(option.code, cachedRate)
        }
        guard option.code != CurrencyOption.usd.code, let sharedRate, sharedRate > 0 else { return }
        guard rate?.rate != sharedRate else { return }
        let next = CurrencyRate(
            code: option.code,
            rate: sharedRate,
            quotedOn: rate?.quotedOn ?? "",
            fetchedAt: rate?.fetchedAt ?? Date()
        )
        rate = next
        defaults.rates[option.code] = next
    }

    private func apply(_ option: CurrencyOption, refresh: Bool = true) {
        selected = option
    ...
    static func lastRefreshSlot(onOrBefore date: Date, calendar: Calendar = .current) -> Date {
        RefreshSchedule.lastRefreshSlot(onOrBefore: date, calendar: calendar)
    }

    /// The next scheduled check strictly after `date`.
    static func nextRefreshSlot(after date: Date, calendar: Calendar = .current) -> Date {
        RefreshSchedule.nextRefreshSlot(after: date, calendar: calendar)
    }
    ...
    private func performRateFetch(for option: CurrencyOption) async {
        ...
        do {
            ...
            guard option == selected else { return }
            rate = next
            onCurrencySelected(next.code, next.rate)
        } catch {
