import type { BudgetConfig, BudgetEvaluation, BudgetStatus } from "./types.js";
...
export function evaluateBudget(
  ...
  const monthlyRatio = config.monthlyBudget && config.monthlyBudget > 0 ? monthlyCost / config.monthlyBudget : null;
  ...
  const status: BudgetStatus = ratios.length === 0
    ? "ok"
    : maximumRatio >= 1
      ? "over"
  ...
  else if (monthlyRatio !== null && monthlyRatio >= config.warningThreshold) reason = "Monthly budget warning";
