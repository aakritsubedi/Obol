...
function positiveOrNull(value: unknown, fallback: number | null): number | null {
  if (value === null || value === undefined || value === "") return fallback;
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
}
...
export function parseConfig(value: unknown): WidgetConfig {
  ...
  return {
    ...
    currencyRate: positiveOrNull(input.currencyRate, DEFAULT_CONFIG.currencyRate),
  };
}
...
export class ConfigStore {
  ...
  async update(patch: Partial<WidgetConfig>): Promise<WidgetConfig> {
    ...
    // A code change cannot keep the previous currency's conversion. Older
    // native clients omit `currencyRate`, so clear it when they change the
    // code; a newer client may provide the replacement rate in the same patch.
    if (
      patch.currency !== undefined &&
      patch.currency !== this.value.currency &&
      patch.currencyRate === undefined
    ) {
      next.currencyRate = null;
    }
    this.value = parseConfig(next);
    await this.save();
    return this.get();
  }
